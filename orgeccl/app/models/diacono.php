...
function model_diacono_funzVescovo($idVescovo){
  ...
  $sql="SELECT p.IdParrocchia,p.Nome, COUNT(f.IdFunzione) as NumeroFunzioni FROM `funzione` f inner join parrocchia p On f.IdParrocchia=p.IdParrocchia WHERE IdVescovo=$idVescovo GROUP BY IdVescovo, IdParrocchia";
